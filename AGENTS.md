...
- **开发验证**：`npm test`（simulate 逻辑断言 + ui-dom/webrtc-dom 沙箱 + verify:notes）+ `node --check` 两个函数文件与 `_shared.js`；**提交前 /code-review 双轴审查**（纯文档编辑豁免）；pre-commit 钩子自动执行（`npm install` 启用）。[工作流](.agents/skills/ip-service-workflow/SKILL.md)
