...
- **生产部署 = 推送 `main`**（控制台 Git 集成，GitHub Provider 项目 CLI 直传被拒）；CLI 直传仅限 Upload Provider 一次性项目且必须带 `-a overseas`（区域参数不持久化，漏带回退 global；区域不可变，换区域须删项目重建）；DNS 全 CNAME 且 Cloudflare 灰云；`4.` 站点关 IPv6。[管理](.agents/notes/implemented/process/2026-08-14-deployment-area-and-domain-management.md)
...
- **开发验证**：`npm test`（simulate 逻辑断言 + ui-dom/webrtc-dom 沙箱 + bundle-gate 打包门禁 + verify:notes）+ `node --check` 两个函数文件与 `_shared.js`；**提交前 /code-review 双轴审查**（纯文档编辑豁免）；pre-commit 钩子自动执行（`npm install` 启用）。[工作流](.agents/skills/ip-service-workflow/SKILL.md)
