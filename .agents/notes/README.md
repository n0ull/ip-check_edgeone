...
理由：项目单语中文、无英文读者，三元组无人消费（早期「未纳入 git 版本管理」的前提已失效——仓库一直受 git 追踪；理由重评见[裁剪理由重评笔记](implemented/process/2026-08-27-note-trimming-rejustification.md)）。
若未来引入英文读者，按三元组契约补齐：`.md` 为英文权威、`.zh.md` 逐节镜像（机器头 `# Agent Note:` 与 `Status:` 保持英文原样）、`.i18n.yaml` 记录配对哈希。
