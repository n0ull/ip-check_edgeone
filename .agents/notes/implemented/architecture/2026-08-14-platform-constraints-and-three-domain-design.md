...
- 其余 Host → 网页；
- `[[default]].js` 提供同逻辑路径端点 `/4` `/6` `/test` `/api/self`（本地调试与兼容保留）。
...
- **保留 `6.ip.<域名>` 作为尽力而为的 IPv6 查询**——它只是 `test.` 的子集（双栈下返回连接 IP），无法保证 IPv6 语义，与 `test.` 重复；移除后功能由 `test.` 完全覆盖，且域名绑定从 4 个减到 3 个。代码中 `case '6'` 与 `/6` 路径端点保留作兼容，文档标注『尽力而为』。
