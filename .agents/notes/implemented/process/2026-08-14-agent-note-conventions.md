...
项目需要统一的决策记录体系（`.agents/`），但成熟开源仓库的完整配套（双语对、格式门禁脚本）对单语小项目过重：双语对无人阅读、脚本无 CI 承载，全部照搬只会制造无人维护的死重。
...
- **事实性同步点**：`getClientIp` 与响应工具函数在 `index.js`/`[[default]].js` 双份内联，修改任一文件必须同步另一份（见[客户端 IP 契约](../architecture/2026-08-14-client-ip-acquisition-contract.md)）。

## Alternatives considered

- **完整照搬成熟开源仓库的配套**（双语对 + 门禁脚本）——双语对在本项目无读者，脚本无 CI 可挂，创建即腐烂；放弃。
...
代价是校验器比完整门禁体系（哈希、配对、跨文档引用门）轻——项目无 git 与 CI，哈希与配对门无承载；若未来纳入 git 与 CI，按 [notes/README.md](../../README.md) 的指引补三元组与完整门禁。