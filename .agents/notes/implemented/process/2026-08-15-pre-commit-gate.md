...
仓库内置 `.githooks/pre-commit`（POSIX sh）：依次执行 `node --check` 两个函数文件与 `npm test`（simulate → webrtc-dom → consistency → verify:notes 全链路，见[测试门禁笔记](../testing/2026-08-15-dual-inline-consistency-and-test-gates.md)），任一失败即阻断提交。
