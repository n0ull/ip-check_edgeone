...
「子域 X 的路径端点是 /X」这份事实在代码里有四份独立维护的副本，中间没有任何断言钉住衔接：
...
推演一次断裂：路由 `/4` 改名 `/v4` → simulate 字面量断言红 → 机械更新该字面量 → 全绿；grab 仍 fetch `/4` → 线上 404 JSON → `looksLikeIp` 判假 → 主页卡片静默「请求失败」。这与同日 esbuild 改名事故（[改名笔记](../bug-fix/2026-08-29-webrtc-free-identifier-esbuild-rename.md)）是同一「本地全绿、线上废」失败类别，且 bundle-gate 的 UI 段只测子域直达路径，对回退路径免疫。

## Decision

`_shared.js` 导出端点路径 fact `subdomainPath(sub)`（`'4'` → `'/4'`、`'test'` → `'/test'`、未知 → `null`），以**函数**形态进 shared 命名空间——页面可经 browserScript 按名拣选、服务端可直接调用（verdictFor 先例）。四方消费：
...
买到的：路径改名只动 fact 一处，switch、grab、hint、三侧测试全部自动跟随；bundle-gate 补上回退路径盲区（10→13 断言）；404 hint 在 fact 变更下永真；「本地绿线上废」类别在端点路径维度被构造性关闭；「同源回退」进入 CONTEXT.md 术语表。
