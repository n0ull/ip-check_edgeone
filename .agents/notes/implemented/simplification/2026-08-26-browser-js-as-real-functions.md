...
- `index.js`：`setStatus` / `showHint` / `looksLikeIp` / `grab(base, sub, label)` / `init(base)` / `isIpv6` / `familyOf` / `verdictFor` / `subdomainPath` → `export const UI_SCRIPT`（后四者分别随[判定表收敛](2026-08-27-verdict-single-source.md)与[端点路径单源化](2026-08-29-endpoint-path-fact-single-sourcing.md)追加，`isIpv6` 是 `familyOf` 的依赖）；`BASE` 由全局前缀行（`var BASE = '__BASE__'`）改为参数化传递，尾行 `init('__BASE__')` 仍由 renderUi 渲染时替换。
- `[[default]].js`：`$` / `addIp` / `isPublicIp` / `extractIp` / `detect` / `fetchIp` / `run` / `isIpv4` / `subdomainPath` → `export const WEBRTC_SCRIPT`（后两者为共享注入：`isIpv4` 随[严格化笔记](../bug-fix/2026-08-27-isipv4-strict-and-regex-dedup.md)成为 `isPublicIp`/`extractIp` 的依赖，`subdomainPath` 随[端点路径单源化](2026-08-29-endpoint-path-fact-single-sourcing.md)被 `run` 的出口 IP fetch 消费）；正则还原正常写法（`[0-9]`→`\d`、`[.]`→`\.`，严格等价），`join('\n')` 恢复字面换行；转义纪律注释删除。
...
- 代价与回评条件：两文件模块顶层出现『服务端不执行』的函数（已有注释标明）；`UI_SCRIPT`/`WEBRTC_SCRIPT` 成为新导出面（EdgeOne 仅消费 `onRequest`，额外导出惰性——若平台未来校验导出白名单需回评）；`toString()` 序列化在平台 esbuild 重印下语义存活（2026-08-27 冒烟实证）——若平台未来启用压缩/改名（minify）则机制失效，升级 CLI 后应重跑部署冒烟回评。
- 后续已闭环：`6.` 兼容面由[移除 6. 兼容面笔记](2026-08-26-remove-v6-compat-surface.md)删除；`/webrtc` 判定分支补齐四路径覆盖（无泄漏 / 泄漏 / 无公网映射 / 无出口，见 `test/webrtc-dom.mjs`）；手工序列化数组由[页面脚本作用域](../architecture/2026-08-29-page-script-scope-and-browserscript.md)的自动闭包取代（闭包缺失类缺陷在构造上消灭），dom-sandbox 全局升级 Proxy 陷阱（模块级标识符泄漏在测试期必现）。
